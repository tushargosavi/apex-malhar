 ...
 * 1. Create a concrete implementation of this operator and implement the method to define the connection
 * properties to Kudu. The connection properties are set using the {@link ApexKuduConnection} using a builder pattern.
 * 2. Implement the logic how tuples need to ne handled in the event of a reconciliation phase ( i.e. when an
 * operator is resuming back from failure ). See javadoc of the method for more details.
 * 3. Define the payload class
 *
 ...
 * simple update pattern thus avoiding a read if required.
 * */
    ...
    implements Operator.ActivationListener<Context.OperatorContext>,Operator.CheckpointNotificationListener
{
  ...
  private ApexKuduConnection apexKuduConnection;
  ...
  @AutoMetric
  transient long numInsertsInThisWindow;

  @AutoMetric
  transient long numUpsertsInThisWindow;

  @AutoMetric
  transient long numDeletedInThisWindow;

  @AutoMetric
  transient long numUpdatesInThisWindow;

  @AutoMetric
  transient long opsErrorsInThisWindow;

  @AutoMetric
  transient long bytesWrittenInThisWindow;

  @AutoMetric
  transient long rpcErrorsInThisWindow;

  @AutoMetric
  transient long writeOpsInThisWindow;

  @AutoMetric
  transient long writeRPCsInThisWindow;
  ...
  private void performCommonProcessing(Operation currentOperation, KuduExecutionContext kuduExecutionContext)
  {
    ...
    for (String columnName: kuduColumnBasedGetters.keySet()) {
      ...
      try {
        ...
        LOG.debug(" Exception while fetching the value of " + columnName + " because " + ex.getMessage());
  ...
  public void beginWindow(long windowId)
  {
    ...
    numDeletedInThisWindow = 0;
    numInsertsInThisWindow = 0;
    numUpdatesInThisWindow = 0;
    numUpsertsInThisWindow = 0;
  }
  ...
  public void endWindow()
  {
    ...
    opsErrorsInThisWindow = kuduClientHandle.getStatistics().getClientStatistic(Statistics.Statistic.OPS_ERRORS) -
      totalOpsErrors;
    bytesWrittenInThisWindow = kuduClientHandle.getStatistics().getClientStatistic(Statistics.Statistic.BYTES_WRITTEN) -
      totalBytesWritten;
    rpcErrorsInThisWindow = kuduClientHandle.getStatistics().getClientStatistic(Statistics.Statistic.RPC_ERRORS) -
      totalRpcErrors;
    writeOpsInThisWindow = kuduClientHandle.getStatistics().getClientStatistic(Statistics.Statistic.WRITE_OPS) -
      totalWriteOps;
    writeRPCsInThisWindow = kuduClientHandle.getStatistics().getClientStatistic(Statistics.Statistic.WRITE_RPCS) -
      totalWriteOps;
  ...
  private void buildGettersForPojoPayload()
  {
    ...
    for ( Field aFieldDef : classFields) {
      ...
      if (allKuduTableColumnNames.contains(currentFieldName)) {
        ...
        if (columnNameOverrides.containsKey(aFieldDef.getName().toLowerCase())) {
          extractGetterForColumn(normalizedColumns.get(columnNameOverrides.get(aFieldDef.getName().toLowerCase())
